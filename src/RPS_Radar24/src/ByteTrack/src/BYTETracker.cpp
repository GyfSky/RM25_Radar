...
BYTETracker::BYTETracker(OurPattern ourPattern): CostMatrix(ourPattern)
{
	track_thresh = 0.25;
	high_thresh = 0.6;
    high_car_thresh = 0.1;
...
void BYTETracker::update(vector<STrack> &tracked_stracks, vector<STrack> &lost_stracks, vector<STrack> &lost_predict_stracks,vector<STrack> &cars, vector<STrack> &out,interfaces::msg::DetectResult lidar_det)
{
...
// self
    ...
    if(lost_predict_stracks_size == lost_stracks_size && lost_predict_stracks_size > 0){
...
//    if(is_lose_predict){
    ...
    std::vector<bool> is_lose(lost_stracks_size, true);  // default 丢失 (is_lose(是否丢失) this mode is true) ,is false 不会进入该帧的lose
    //更新丢失状态
    for (int i = 0; i < matches.size(); i++)
    {
        ...
        is_lose[lose_track_list_index] = false;
...
//                    std::cout << "always_frame_lost_predict_stracks[x].Locate3D: " << always_frame_loalways_frame_lost_predict_stracksst_predict_stracks[x].Locate3D.x <<  "  " << always_frame_lost_predict_stracks[x].Locate3D.y << std::endl;
        ...
        std::vector<double> max_conf(classWithoutCar,0.0);
        std::vector<int> max_ut_idx(classWithoutCar,-1);
        ...
        for (int i=0;i<half_classWithoutCar;i++) {
            ...
            if (strack_pool[u_strack[max_ut_idx[i]]]->lost_frame_ind_num>0) {
               strack_pool[u_strack[max_ut_idx[i]]]->lost_frame_ind_num--;
            }
...
void BYTETracker::update(vector<STrack> &tracked_stracks, vector<STrack> &lost_stracks, vector<STrack> &lost_predict_stracks,vector<STrack> &cars, vector<STrack> &out) {
    //tracked_stracks 里面为 activated_stracks refind_stracks
    ////////////////// Step 1: Get detections //////////////////
    this->frame_id++; //TODO：
    vector<STrack> activated_stracks;//更新状态为跟踪的跟踪器 新建的跟踪器 当前帧更新过的跟踪器
        ...
        for (int i=0; i < matches_cls.size(); i++)
        {
            ...
            if(is_lose_predict){
                ...
                if(x >-1){
                    ...
                    always_frame_lost_predict_stracks[x].track_id = strack_pool[index_stracks_pool]->track_id;//？？
                    //                    std::cout << "index_stracks_pool: " <<index_stracks_pool << ", " << cls_stracks_pool << std::endl;
                    //                    std::cout << "always_frame_lost_predict_stracks[x].Locate3D: " << always_frame_lost_predict_stracks[x].Locate3D.x <<  "  " << always_frame_lost_predict_stracks[x].Locate3D.y << std::endl;
                    if(classWithoutCar ==14){
                        if(cls_stracks_pool == half_classWithoutCar -1 || cls_stracks_pool == classWithoutCar - 1){
                            continue;
                        }
...
void BYTETracker::update(vector<STrack> &tracked_stracks, vector<STrack> &lost_stracks, vector<Car> &cars)
{
    ////////////////// Step 1: Get detections //////////////////
    this->frame_id++; //TODO：
    ...
    vector<STrack> removed_stracks;
    vector<STrack> current_frame_lost_stracks;
...
//    vector<STrack> output_stracks;
    ...
    vector<STrack*> current_frame_tracked_stracks;
    vector<STrack*> strack_pool;
    ...
    if (cars.size() > 0)
    {
        for (int i = 0; i < cars.size(); i++)
        {
            vector<float> tlbr_;
            tlbr_.resize(4);
            tlbr_[0] = cars[i].rect.x;
            tlbr_[1] = cars[i].rect.y;
            tlbr_[2] = cars[i].rect.x + cars[i].rect.width;
            tlbr_[3] = cars[i].rect.y + cars[i].rect.height;


            STrack strack(STrack::tlbr_to_tlwh(tlbr_), cars[i]);
            if(strack.cls >12){
//                std::cout << "strack.cls: " << strack.cls << std::endl;
//                std::cout << "strack.track_id: " << strack.track_id << std::endl;
//                std::cout << "strack.conf: " << strack.conf << std::endl;
            }
            if (strack.conf >= track_thresh)
            {
...
//    std::cout << "cost_confMatrix _ cls:  " << std::endl << cost_confMatrix << std::endl;
    ...
    for (int i=0; i < matches_cls.size(); i++)
    {
        strack_pool[matches_cls[i][0]]->cls = matches_cls[i][1];
        strack_pool[matches_cls[i][0]]->conf_armor = strack_pool[matches_cls[i][0]]->ws_armorConfMatrix(0,matches_cls[i][1]);
        strack_pool[matches_cls[i][0]]->updata_trackid(num);
    }
...
////instead ing
    vector<vector<float> > dists;
    int dist_size = 0, dist_size_size = 0;//dist_size = atracks.size(); dist_size_size = btracks.size();
//    dists = iou_distance(strack_pool, detections, dist_size, dist_size_size); // cost matrix
//    dists = getIouAndDistancetCost_vec(strack_pool, detections, dist_size, dist_size_size, false);
    Eigen::MatrixXd cost_matrix = getIouAndDistancetCost(strack_pool, detections, dist_size, dist_size_size, false);
    eigenMat2VecVec(cost_matrix,dists);
//    std::cout << "cost_matrix_strack_pool:  "<< std::endl << cost_matrix <<std::endl;
    vector<vector<int> > matches;
    vector<int> u_track, u_detection;
    std::cout << "cost_matrix2\n" << cost_matrix << std::endl;
    linear_assignment(dists, dist_size, dist_size_size, match_thresh, matches, u_track, u_detection);
...
//    dists = getIouAndDistancetCost_vec(r_tracked_stracks, detections, dist_size, dist_size_size, false);
    cost_matrix = getIouAndDistancetCost(r_tracked_stracks, detections, dist_size, dist_size_size, false);
    eigenMat2VecVec(cost_matrix,dists);
...
//    dists = getIouAndDistancetCost_vec(unconfirmed, detections, dist_size, dist_size_size, false);
    cost_matrix = getIouAndDistancetCost(unconfirmed, detections, dist_size, dist_size_size, false);
    eigenMat2VecVec(cost_matrix,dists);
    ...
    for (int i = 0; i < matches.size(); i++)
    {
        unconfirmed[matches[i][0]]->update(detections[matches[i][1]], this->frame_id,ourPattern, windmill_car);
    ...
    {
        STrack *track = &detections[u_detection[i]];
        if (track->conf_armor < this->high_thresh && (track->conf_armor > 1e-6 && track->conf < this->high_car_thresh) )
            continue;
        track->activate(this->kalman_filter, this->frame_id,num);
        activated_stracks.push_back(*track);
    }
    ...
    for (int i = 0; i < tracked_stracks.size(); i++)
    {
        ...
        {
            tracked_stracks_swap.push_back(tracked_stracks[i]);
        }
    }
    tracked_stracks.clear();
    tracked_stracks.assign(tracked_stracks_swap.begin(), tracked_stracks_swap.end());
    ...
    // lost_stracks = sub_stracks(lost_stracks, tracked_stracks, false);
    for (int i = 0; i < current_frame_lost_stracks.size(); i++)
    {
        lost_stracks.push_back(current_frame_lost_stracks[i]);
    }

    std::cout << "step5" << std::endl;


    lost_stracks = sub_stracks(lost_stracks, this->removed_stracks);
    // lost_stracks = sub_stracks(lost_stracks, this->removed_stracks, false);
    ...
    remove_duplicate_stracks(resa, resb, tracked_stracks, lost_stracks);

    tracked_stracks.clear();
    tracked_stracks.assign(resa.begin(), resa.end());
    lost_stracks.clear();
    for(int i = 0; i < resb.size(); i++){
        if(resb[i].Locate2D.x>1440 || resb[i].Locate2D.x<0 || resb[i].Locate2D.y<0 || resb[i].Locate2D.y>1080){
            continue;
        }else if(resb[i].track_id < 20){
            lost_stracks.push_back(resb[i]);
        }
    }
    lost_stracks.assign(resb.begin(), resb.end());
...
//    for (int i = 0; i < tracked_stracks.size(); i++)
//    {
//        if (tracked_stracks[i].is_activated) // TODO: when is not is_activated
//        {
//            output_stracks.push_back(tracked_stracks[i]);
//        }
////        std::cout << "tracked_stracks[i]->tracklet_len:   " << tracked_stracks[i]->tracklet_len << std::endl;
//    }
//
//    cost_confMatrix = getCost_confMatrix(output_stracks,num_strack,num_cls);
//    eigenMat2VecVec(cost_confMatrix, cost_confMatrix_vec);
//    std::cout << "cost_matrix/n..." << std::endl <<  cost_matrix << std::endl;
//    linear_assignment(cost_confMatrix_vec, num_strack, num_cls, match_thresh, matches_cls, u_strack, u_cls);
//
//    for (int i=0; i < matches_cls.size(); i++)
//    {
//        if( -1 < matches_cls[i][1] && matches_cls[i][1] <14){
//            output_stracks[matches_cls[i][0]].cls = matches_cls[i][1];
//            output_stracks[matches_cls[i][0]].conf_armor = output_stracks[matches_cls[i][0]].ws_armorConfMatrix(0,matches_cls[i][1]);
//        }
//    }
//    num = 0;
//    for(int i=0; i < u_strack.size();i++){
//        classfy_STrack_N(output_stracks[u_strack[i]],num);
//    }
//
//    std::cout << "."  ;
//    output_stracks.insert(output_stracks.end(), lost_stracks.begin(), lost_stracks.end());
//    if (lost_stracks.size() != 0){
